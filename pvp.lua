...
local _UIReady = false -- chặn callback chạy lúc init
...
-- Thông báo đang load (sau khi PlayerGui đã sẵn sàng)
task.delay(0.2, function()
    _Notify("Xico Hub", "⏳ Đang tải menu...", 5, "info")
end)
...
local AimlockPlayersToggle = SectionAim:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Aimlock Players" or "❌ Tắt: Aimlock Players"), 3, state and "success" or "error")
        end
    end
})
...
local AimlockPlayersMiniTogglePlayersToggle = SectionAim:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Aimlock Mini Toggle Players" or "❌ Tắt: Aimlock Mini Toggle Players"), 3, state and "success" or "error")
        end
    end
})
...
local AimlockNPCToggle = SectionAim:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Aimlock NPC" or "❌ Tắt: Aimlock NPC"), 3, state and "success" or "error")
        end
    end
})
...
local AimlockPlayersMiniToggleNPCToggle = SectionAim:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Aimlock Mini Toggle NPC" or "❌ Tắt: Aimlock Mini Toggle NPC"), 3, state and "success" or "error")
        end
    end
})
...
local PredictionToggle = SectionAim:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Prediction" or "❌ Tắt: Prediction"), 3, state and "success" or "error")
        end
    end
})
...
local SilentAimPlayersToggle = SectionSilent:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: SilentAim Players" or "❌ Tắt: SilentAim Players"), 3, state and "success" or "error")
        end
    end
})
...
local SilentMiniTogglePlayersToggle = SectionSilent:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: SilentAim Mini Toggle Players" or "❌ Tắt: SilentAim Mini Toggle Players"), 3, state and "success" or "error")
        end
    end
})
...
local SilentAimNPCToggle = SectionSilent:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: SilentAim NPCs" or "❌ Tắt: SilentAim NPCs"), 3, state and "success" or "error")
        end
    end
})
...
local SilentMiniToggleNPCToggle = SectionSilent:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: SilentAim Mini Toggle NPC" or "❌ Tắt: SilentAim Mini Toggle NPC"), 3, state and "success" or "error")
        end
    end
})
...
local SilentAimPedictionToggle = SectionSilent:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: SilentAim Prediction" or "❌ Tắt: SilentAim Prediction"), 3, state and "success" or "error")
        end
    end
})
...
local ZSkillToggle = SectionSilent:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: GodhumanZ Aimlock" or "❌ Tắt: GodhumanZ Aimlock"), 3, state and "success" or "error")
        end
    end
})
...
local HighlightToggle = SectionSilent:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Main Highlight" or "❌ Tắt: Main Highlight"), 3, state and "success" or "error")
        end
    end
})
...
local ZskillMOneToggle = SectionSilent:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Z|M1 Skills" or "❌ Tắt: Z|M1 Skills"), 3, state and "success" or "error")
        end
    end
})
...
local ESPPlayersToggle = SectionFeatures:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: ESP Players" or "❌ Tắt: ESP Players"), 3, state and "success" or "error")
        end
    end
})
...
local BunnyHopToggle = SectionFeatures:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Bunny Hop" or "❌ Tắt: Bunny Hop"), 3, state and "success" or "error")
        end
    end
})
...
local AuraSkillToggle = SectionFeatures:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Aura Skill" or "❌ Tắt: Aura Skill"), 3, state and "success" or "error")
        end
    end
})
...
local FpsOrPingsToggle = SectionFeatures:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Fps Or Pings" or "❌ Tắt: Fps Or Pings"), 3, state and "success" or "error")
        end
    end
})
...
local SprintPercentInput = SectionFeatures:AddInput({
    Title = "Sprint %",
    Description = "10 = bình thường, >10 chạy nhanh hơn",
    PlaceHolder = "10",
    Default = "",
...
local SprintToggle = SectionFeatures:AddToggle({
    Title = "Sprint (Shift)",
    Description = "Giữ Shift để chạy nhanh",
    Default = false,
    Callback = function(state)
        SprintEnabled = state
        Settings["SprintShift"] = state
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Sprint (Shift)" or "❌ Tắt: Sprint (Shift)"), 3, state and "success" or "error")
        end
        UpdateSprintSpeed()
    end
})
...
local SpeedSlider = SectionFeatures:AddSlider({
    Title = "Speed Hack",
    Description = "Keo de chinh WalkSpeed (16 = binh thuong)",
    Min = 16,
    Max = 150,
    Increment = 1,
    Default = 16,
...
local FpsBoostToggle = SectionFeatures:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Fps Boost" or "❌ Tắt: Fps Boost"), 3, state and "success" or "error")
        end
    end
})
...
local INFEnergyToggle = SectionFeatures:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: INF Energy" or "❌ Tắt: INF Energy"), 3, state and "success" or "error")
        end
    end
})
...
local WalkonWaterToggle = SectionFeatures:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Walk on Water" or "❌ Tắt: Walk on Water"), 3, state and "success" or "error")
        end
    end
})
...
local FastAttackToggle = SectionFeatures:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Fast Attack" or "❌ Tắt: Fast Attack"), 3, state and "success" or "error")
        end
    end
})
...
local AntiAFKToggle = SectionFeatures:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Anti AFK" or "❌ Tắt: Anti AFK"), 3, state and "success" or "error")
        end
    end
})
...
local FruitCheckToggle = SectionFeatures:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Spawned Fruit Check" or "❌ Tắt: Spawned Fruit Check"), 3, state and "success" or "error")
        end
    end
})
...
local TeleportFruitToggle = SectionFeatures:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Bring Fruits" or "❌ Tắt: Bring Fruits"), 3, state and "success" or "error")
        en
    end
})
...
local AutoKenToggle = SectionFeatures:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Auto Ken" or "❌ Tắt: Auto Ken"), 3, state and "success" or "error")
        end
    end
})
...
local LavaToggle = SectionFeatures:AddToggle({
    ...
    Callback = function(state)
        ...
        Settings["Lava"] = state
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Remove Lava" or "❌ Tắt: Remove Lava"), 3, state and "success" or "error")
        end
    end
})
...
local FogToggle = SectionFeatures:AddToggle({
    ...
    Callback = function(state)
        ...
        Settings["Fog"] = state
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Remove Fog" or "❌ Tắt: Remove Fog"), 3, state and "success" or "error")
        end
    end
})
...
local DodgeToggle = SectionFeatures:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Dodge No CD" or "❌ Tắt: Dodge No CD"), 3, state and "success" or "error")
        end
    end
})
...
local OpponentToggle = SectionFeatures:AddToggle({
    ...
    Callback = function(state)
        ...
        if _UIReady then
            _Notify("Xico Hub", (state and "✅ Bật: Target Info" or "❌ Tắt: Target Info"), 3, state and "success" or "error")
        end
    end
})
...
-- Bật flag sau khi UI đã load xong hoàn toàn
-- Từ đây các dropdown mới được phép gọi callback
_UIReady = true

-- Thông báo load xong
_Notify("Xico Hub", "✅ Menu đã tải xong! Chúc bạn chơi vui.", 5, "success")

-- ════════════════════════════════════════════
--           WEBHOOK NOTIFIER (chạy ngầm)
-- ════════════════════════════════════════════
